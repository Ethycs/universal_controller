...
| `bench/` | Generalization benchmark: offline MHTML fixtures of real chat sites, honest top-1 scoring, regression gate ([bench/README.md](bench/README.md)) |

---

## Measuring generalization

The "any chat UI" claim is tested, not assumed. `bench/` captures real
sites as MHTML snapshots (full CSS/ARIA fidelity, iframes included),
injects the built engine offline via CDP, and scores whether UC's **#1**
answer for the input, send button, and message stream matches
per-site ground-truth oracles — plus false-positive checks on search and
form pages where there is no chat. Deterministic, login-free, CI-able.

```bash
pixi run bench            # score all fixtures
pixi run bench-check      # fail on regression vs the checked-in baseline
pixi run bench-capture    # refresh fixtures from the live sites
```
